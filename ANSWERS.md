...
### Accessibility:
- Required fields are marked only by an asterisk - weak indication for screen reader users.
...
Preconditions: user is on the billing screen, valid test card available.
...
Preconditions: user is on the billing screen.
...
The two most severe bugs I've found are no CVV field, and card data being collected to the company's servers.

These bugs are critical for two main reasons: a) Without CVV, the form can't verify that the user actually holds the card. b) Collecting raw card data puts the company's servers in full PCI-DSS scope — every system touching the data must meet strict security requirements and be audited.
